...
| 前端 | 原生 HTML/CSS/JS（单页应用） | Linear/Vercel 风格设计系统，侧边栏 + 对话区 + 进度面板 + 文件面板 |
...
### 3.5 Web UI 前端（static/index.html）

单页应用（~1520 行），Linear/Vercel 风格设计系统。使用原生 HTML/CSS/JS，无前端框架。

#### 3.5.1 设计系统

| 变量 | 值 | 用途 |
|------|-----|------|
| `--sidebar-bg` | `#0f172a` | 侧边栏深色背景 |
| `--main-bg` | `#f8fafc` | 主内容区浅色背景 |
| `--accent` | `#3b82f6` | 主色调（蓝色） |
| `--user-bubble` | `#3b82f6` | 用户消息气泡（蓝底白字） |
| `--agent-bubble` | `#ffffff` | Agent 回复气泡（白底深字，带边框） |
| `--progress-bg` | `#0f172a` | 进度面板（终端风格深色） |
...
│  [排序切换]  │  │  用户消息（蓝底气泡）              │    │
│              │  │                                   │    │
│  Current Run│  │  Agent 回复（白底气泡 + Markdown） │    │
│  [隐藏面板]  │  │                                   │    │
│              │  └─────────────────────────────────┘    │
│  Actions    │                                         │
│  [市场分析]  │  ┌─────────────────────────────────┐    │
...
| 区域 | 行为 |
|------|------|
| **Workflow 按钮** | 点击 "Find & Match Jobs" 触发 `/api/pipeline`（search_match），一键执行三步流水线 |
| **排序切换** | Workflow 下方的 radio 组，切换 `sort_by`：按日期（最新在前）或按相关度（JobsDB 默认排序） |
| **模型切换** | `<select>` 下拉框，列出所有 provider 的默认模型。选择后 POST `/api/config/model`，立即生效。切换成功显示 ✓ |
| **Current Run** | 当前活跃 run 的面板（初始隐藏），显示运行时间和岗位统计 |
| **Actions 区域** | "Market Analysis..." 按钮弹出市场分析参数输入框；可折叠的 "Generate Resume" 分组（展开后含三个子按钮：From JD Text / By Role Direction / General Resume） |
| **History** | 从 `/api/runs` 获取历史 run 列表，当前活跃 run 高亮。点击可查看该 run 的文件列表 |
| **进度面板** | SSE 实时流，终端风格（黑底绿字），显示 `emit()` 的所有输出 + 工具调用状态。支持自动滚动到底部 |
| **文件面板** | 列出 `output/` 中文件，点击触发 `/download/<path>` 下载 |
| **Market Reports** | 列出所有市场分析报告文件，支持点击下载 |
| **Markdown 渲染** | Agent 回复中的 Markdown 语法（标题、列表、粗体、代码块）在对话区做格式化渲染 |

#### 3.5.4 SSE 流处理

```javascript
// 前端连接 SSE 流
const source = new EventSource(`/stream/${sid}`);
source.addEventListener('message', (e) => {
    const data = JSON.parse(e.data);
    switch (data.type) {
        case 'progress': // 追加到进度面板
        case 'tool_call': // 显示工具调用状态
        case 'done':      // 显示 Agent 回复 + 文件列表
        case 'error':     // 显示错误
        case 'ping':      // 忽略，保持连接
    }
});
...
2. **侧边栏 → Model 下拉框**：选择 LLM provider（deepseek/qwen/glm），立即生效，切换成功显示 ✓
3. **侧边栏 → Workflow → "Find & Match Jobs"**：一键触发 search→match→resume 三步流水线（无需打字）
4. **侧边栏 → Workflow → 排序切换**：选择按日期（最新在前）或按相关度排序
5. **侧边栏 → Actions → "Market Analysis..."**：弹出输入框，填写岗位类别等参数后提交
6. **侧边栏 → Actions → "Generate Resume"**（可折叠分组）：展开后选择简历生成模式
7. **对话框**：直接输入指令（同终端模式），SSE 实时显示进度和工具调用状态
8. **进度面板**：终端风格日志滚动显示，包含所有 `emit()` 输出
9. **侧边栏 → History**：浏览所有历史 run，点击查看详细文件列表
10. **侧边栏 → Files**：浏览所有 output 文件，点击下载
11. **侧边栏 → Reports**：浏览所有市场分析报告，点击下载
