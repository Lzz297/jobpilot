...
| 区域 | 行为 |
|------|------|
| **快速按钮** | 点击直接触发 `/api/pipeline`（search_match），Sidebar 中的「找工作」按钮 |
| **模型切换** | `<select>` 下拉框，列出所有 provider 的默认模型。选择后 POST `/api/config/model`，立即生效 |
| **Run 列表** | 从 `/api/runs` 获取，显示为可点击条目，当前活跃 run 高亮。点击可查看该 run 的文件列表 |
| **进度面板** | SSE 实时流，终端风格（黑底绿字），显示 `emit()` 的所有输出 + 工具调用状态。支持自动滚动到底部 |
| **文件面板** | 列出 `output/` 中文件，点击触发 `/download/<path>` 下载 |
| **Market 报告** | 每条报告带删除按钮（`/api/market/files` + 前端删除逻辑） |
| **智能折叠** | 侧边栏区域可按需折叠/展开，减少信息过载 |
...
| `REPORT_CSS` | 市场分析报告 | 深蓝 `#1a5276` 标题色、表格样式（斑马纹：`#f2f2f2` 交替行色）、更宽间距 |
...
2. **侧边栏 → 模型切换**：下拉框选择 LLM provider（deepseek/qwen/glm），立即生效
3. **侧边栏 → 快速按钮**：一键触发「找工作」全流程（无需打字）
4. **对话框**：直接输入指令（同终端模式），SSE 实时显示进度和工具调用状态
5. **进度面板**：终端风格日志滚动显示，包含所有 `emit()` 输出
6. **侧边栏 → 当前运行**：查看当前活跃 run 的阶段和岗位数量
7. **侧边栏 → 历史记录**：浏览所有历史 run，点击查看详细文件列表
8. **侧边栏 → 文件面板**：浏览所有 output 文件，点击下载
9. **侧边栏 → 市场报告**：浏览所有市场分析报告，支持一键删除
